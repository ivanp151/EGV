...
if ( ! function_exists('menu_ul'))
{
    function menu_ul($sel = 'home')
   {
        ...
        {
            $current = (in_array($sel, $item)) ? ' class="current berlin border-litle " '  : 'class="item-nav berlin border-litle"';
            $id = (!empty($item['id'])) ? ' id="'.$item['id'].'"' : '';
            $last = ($item['id']=='photos')? 'style="margin-right:0px"' : '';
            $menu .= '<li><a '.$current.' '.$last.' href="'.$item['link'].'"'.$id.'>'.$item['title'].'</a></li>'."\n";
        }
        $menu .= '</ul>'."\n";
        return $menu;
    }
